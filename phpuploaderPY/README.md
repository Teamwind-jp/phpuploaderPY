...
1.メイン処理は、mainThread()内に書いています。threadingで別スレッドにしています。  
...
5.対象フォルダをzip後に指定サイズにカット。各ファイルをrequestsライブラリにて順次アップロードしています。  
6.php側は、受信したファイルのmd5値をprmと比較、最終ファイルなら結合してzipに戻しています。  
